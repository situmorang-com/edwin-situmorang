...
export function initSync() {
	...
	syncInterval = window.setInterval(checkAndSync, 30000); // Every 30 seconds
...
export async function addEntryWithSync(entry: Omit<FeedingEntry, 'id' | 'created_at'>) {
	const authState = get(auth);
	...
	if (navigator.onLine) {
		...
		try {
			...
			// Save to IndexedDB
			await idb.saveEntries([serverEntry, ...await idb.getAllEntries()]);
